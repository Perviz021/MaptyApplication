...
class Workout {
  // public fields
  date = new Date();
  id = (Date.now() + "").slice(-10); // random id creating using Date
  clicks = 0;
...
class App {
  ...
  constructor() {
    ...
    UI.containerWorkouts.addEventListener(
      "click",
  ...
  _loadMap(position) {
    ...
    // Adding markers after the map is loaded fully
  ...
  _newWorkout(e) {
    ...
    const distance = +UI.inputDistance.value;
    const duration = +UI.inputDuration.value;
    const { lat, lng } = this.#mapEvent.latlng;
    let workout;

    // If workout running, create running object
    if (type === "running") {
      const cadence = +UI.inputCadence.value;
    ...
    if (type === "cycling") {
      const elevation = +UI.inputElevation.value;
      // Check if data is valid
      if (
  ...
  _renderWorkout(workout) {
    ...
    UI.form.insertAdjacentHTML("afterend", html);
  }
  ...
  _moveToPopup(e) {
    ...
    // Leaflet method to move marker on the map
